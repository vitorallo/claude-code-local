# Running Claude Code on a local model (Apple Silicon)

The long version: install, the model catalog, memory budgeting, the Qwen3.8-27B
runbook, every flag `cclocal` takes — and the catalogue of **32 problems** hit
along the way, each with the measurement behind it and what fixes it.

Short version, and the hosted-provider path: [../README.md](../README.md).

> Paths in this document are relative to the repo root, one level up.
...
### Hosted providers via ccrouter

`cclocal --ccrouter` routes the session to a hosted OpenAI-compatible provider
instead of a local model, and plugs into the same remote path as above — the
launcher reads the profile's context window and output cap from the router's
`/v1/models`. See [ccrouter/README.md](../ccrouter/README.md).
...
| `--ccrouter [N\|R]` | Start ccrouter on port 8787 with profile N, a random one (R), or one chosen from a menu (no value), and use hosted OpenAI-compatible providers (see [ccrouter/README.md](../ccrouter/README.md)) |
